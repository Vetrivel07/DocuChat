...
from src.utils.embed_cache import (
    CacheDb,
    lookup_existing_vectors,
    fetch_vectors,
    upsert_vectors,
    lookup_existing_doc_chunks,
    insert_doc_vectors,
)
...
def _embed_text_for_chunk(chunk: Dict[str, Any]) -> str:
    ...
    Prefer embedding_text (enriched with section context).
    Fall back to text_clean for backwards compat with old chunks.
    """
    et = (chunk.get("embedding_text") or "").strip()
    if et:
        return et
    return (chunk.get("text_clean") or "").strip()


def _cache_key(*, embedder_id: str, embed_text: str) -> str:
    """
    Content-based cache key uses the actual text that gets embedded.
    This ensures cache hits when embedding_text is consistent.
    """
    sig = _embed_signature(embedder_id)
    fp  = _sha256_hex(embed_text)
    return _sha256_hex(f"{sig}|{fp}")


def _should_index(chunk: Dict[str, Any]) -> bool:
    """
    Returns True if this chunk should be embedded and indexed.
    Chunks with index_this=False (references, caption-only figures) are skipped.
    """
    # Default True for backwards compat with old chunks that lack index_this
    return bool(chunk.get("index_this", True))


def _make_batches(
    items: List[Dict[str, Any]], cfg: EmbedBatching
) -> Iterable[List[Dict[str, Any]]]:
    batch: List[Dict[str, Any]] = []
    chars = 0

    for it in items:
        t    = _embed_text_for_chunk(it)
        tlen = len(t)

        if batch and (
            len(batch) >= cfg.max_batch_size
            or (chars + tlen) > cfg.max_batch_chars
        ):
            yield batch
            batch = []
            chars = 0
...
def _build_vector_row(b: Dict[str, Any], v: List[float], embedder: Embedder, ts: str) -> Dict[str, Any]:
    """Build the vector JSONL row — carries all metadata downstream."""
    return {
        "chunk_id":        b["chunk_id"],
        "doc_id":          b["doc_id"],
        ...
        "section_context": b.get("section_context", ""),
        "chunk_type":      b.get("chunk_type", "semantic_text"),
        "token_count":     b.get("token_count", 0),
        "index_this":      b.get("index_this", True),
        "vector":          v,
        "dim":             embedder.dim,
...
# ══════════════════════════════════════════════════════════════════════════════
# v1 — JSONL scan + merge/replace
# ══════════════════════════════════════════════════════════════════════════════

def _existing_chunk_ids(
    vectors_path: Path, *, bench: EmbedBench | None = None
) -> Set[str]:
...
def _iter_missing_chunks_v1(
    ...
    *,
    ...
    for ch in iter_jsonl(chunks_path):
        ...
        # Skip chunks flagged as not to be indexed
        if not _should_index(ch):
            skipped += 1
            continue

        embed_text = _embed_text_for_chunk(ch)
        if not embed_text:
            skipped += 1
            continue
...
def embed_doc_chunks(
    *,
    ...
    """v1: JSONL scan skip; merge+replace."""
    ...
    for batch in _make_batches(missing_chunks, batching):
        ...
        texts = [_embed_text_for_chunk(b) for b in batch]
        ...
        for b, v in zip(batch, vecs):
            ...
            new_rows.append(_build_vector_row(b, v, embedder, ts))
...
def embed_doc_chunks_sqlite(
    *,
    ...
    """
    v2: SQLite-backed content dedup.
    Cache key is based on embedding_text (not text_clean) so enriched
    embedding text is properly cached and reused.
    """
    bench and bench.set("dim_expected", embedder.dim)
    ...
    try:
        ...
        for ch in iter_jsonl(chunks_path):
            ...
            # Skip non-indexed chunks (references, caption-only figures)
            if not _should_index(ch):
                continue

            embed_text = _embed_text_for_chunk(ch)
            if not cid or not doc_id or not embed_text:
                continue

            # Cache key uses actual embed text
            ch["_cache_key"] = _cache_key(
                embedder_id=embedder.embedder_id,
        ...
        if hit_chunks:
            ...
            vec_map = fetch_vectors(con, [str(ch["_cache_key"]) for ch in hit_chunks])
            ...
            for ch in hit_chunks:
                ...
                row = _build_vector_row(ch, vec, embedder, ts)
                row["cache_key"] = ck
                rows_hit.append(row)
                doc_rows_hit.append((
                    str(ch["doc_id"]), str(ch["chunk_id"]),
                    embedder.embedder_id, ck,
                    str(vectors_path), int(embedder.dim), ts,
                ))
        ...
        if bench:
            ...
            bench.set("sqlite_hits",     len(rows_hit))
        ...
        for batch in _make_batches(miss_chunks, batching):
            ...
            # Embed enriched text
            texts = [ch["_embed_text"] for ch in batch]
            vecs  = embedder.embed_texts(texts)
            if len(vecs) != len(batch):
                raise ValueError("EMBEDDING_SIZE_MISMATCH")
            ...
            for b, v in zip(batch, vecs):
                ...
                ck  = str(b["_cache_key"])
                row = _build_vector_row(b, v, embedder, ts)
                row["cache_key"] = ck
                rows_out.append(row)
