...
def _safe_dir_name(embedder_dir: str) -> str:
    # you already use safe_name() elsewhere, so prefer that in runner.
...
def build_faiss_for_collection(
    *,
    collection_id: str,
    vectors_dir: Path,   # storage/vectors
    indexes_dir: Path,   # storage/indexes
    embedder_id: str,
    embedder_dir: str,   # safe directory name (use safe_name(embedder_id))
    dim: int,
    metric: str,         # "ip" or "l2"
    normalized: bool,
) -> IndexResult:
    ...
    doc_files = sorted([p for p in src_dir.glob("*.jsonl") if p.is_file()], key=lambda p: p.stem)
