...
from src.retrieval.embedder_id import parse_embedder_id  # you must have this
...
def _load_chunk_text_map(chunks_jsonl: Path) -> Dict[str, str]:
    out: Dict[str, str] = {}
    for row in _iter_jsonl(chunks_jsonl):
        cid = row.get("chunk_id")
        txt = row.get("text_clean") or row.get("text_original") or ""
        if cid and txt:
            out[str(cid)] = str(txt)
    return out
...
class VectorRetriever:
    s: Settings
    _faiss_cache: Dict[str, Any] = None  # small optional cache
    ...
    def retrieve(
        ...
        *,
        ...
        doc_text_maps: Dict[str, Dict[str, str]] = {}
        for doc_id in need_doc_ids:
            p = self.s.chunks_dir / active.collection_id / f"{doc_id}.jsonl"
            doc_text_maps[doc_id] = _load_chunk_text_map(p) if p.exists() else {}
        ...
        for pos, row in enumerate(picked_meta):
            ...
            text = doc_text_maps.get(doc_id, {}).get(chunk_id, "")
