...
# 3) v2/v1 switch stays local to this function
    ...
    def _graph(self, job_id: str) -> None:
        s = get_settings()
        job = self.store.read_job(job_id)

        manifest = load_manifest(s.collections_dir, job.collection_id)
        retrieval_mode = str(manifest.get("retrieval_mode") or "vector").strip().lower()

        if retrieval_mode != "hybrid":
            return

        if not s.graph.enabled or not s.graph.build_enabled:
            return

        builder = GraphBuilder(s)
        if s.graph.clean_before_build:
            from src.graph.factory import get_graph_store
            store = get_graph_store(s)
            try:
                store.purge_collection(job.collection_id)
            finally:
                store.close()
        builder.build(job.collection_id)
    ...
    def run(self, job_id: str) -> None:
        ...
        try:
            ...
            self.store.stage_running(job, Stage.GRAPH.value)
            self._graph(job_id)
            job = self.store.read_job(job_id)
            self.store.stage_done(job, Stage.GRAPH.value)

            any_success = any(d.doc_id and d.status == "done" and not d.fail_reason for d in job.docs)
            if not any_success:
                self.store.fail_job(job, Stage.GRAPH.value, "ALL_DOCS_FAILED_BY_EMBED")
                return

            self.store.stage_running(job, Stage.DONE.value)
            job = self.store.read_job(job_id)
            self.store.stage_done(job, Stage.DONE.value)

            job = self.store.read_job(job_id)
            job.status = "done"
            job.current_stage = Stage.DONE.value
            self.store.write_job(job)
