...
class RetrievalConfig:
    ...
    hybrid_graph_weight: float = 0.35
...
class GraphConfig:
    ...
    entity_min_confidence: float = 0.70
    relation_min_confidence: float = 0.75
    enable_section_nodes: bool = True

    extractor: str = "neo4j_graphrag"
    llm_provider: str = "openai"
    llm_model: str = "gpt-4o-mini"
    llm_temperature: float = 0.0

    schema_mode: str = "EXTRACTED"   # EXTRACTED | FREE
    embedding_model: str = "text-embedding-3-small"
    build_batch_size: int = 1
    source_link_mode: str = "from_chunk"
...
class Settings:
    ...
    @property
    def graphs_dir(self) -> Path: return self.storage_root / "graphs"
...
def get_settings() -> Settings:
    ...
    retrieval = RetrievalConfig(
        default_mode=_env_str("RETRIEVAL_MODE", "vector").strip().lower(),
        ...
        graph_top_k=max(1, _env_int("GRAPH_TOP_K", 5)),
        hybrid_vector_weight=_env_float("HYBRID_VECTOR_WEIGHT", 0.65),
        hybrid_graph_weight=_env_float("HYBRID_GRAPH_WEIGHT", 0.35),
    )
    ...
    graph = GraphConfig(
        enabled=_env_bool("GRAPH_ENABLED", True),
        provider=_env_str("GRAPH_PROVIDER", "neo4j"),
        uri=_env_str("GRAPH_URI", "bolt://127.0.0.1:7687"),
        user=_env_str("GRAPH_USER", "neo4j"),
        password=_env_str("GRAPH_PASSWORD", ""),
        database=_env_str("GRAPH_DATABASE", "neo4j"),
        build_enabled=_env_bool("GRAPH_BUILD_ENABLED", True),
        max_relation_hops=_env_int("GRAPH_MAX_RELATION_HOPS", 2),
        entity_min_confidence=_env_float("GRAPH_ENTITY_MIN_CONFIDENCE", 0.70),
        relation_min_confidence=_env_float("GRAPH_RELATION_MIN_CONFIDENCE", 0.75),
        enable_section_nodes=_env_bool("GRAPH_ENABLE_SECTION_NODES", True),
        extractor=_env_str("GRAPH_EXTRACTOR", "neo4j_graphrag"),
        llm_provider=_env_str("GRAPH_LLM_PROVIDER", "openai"),
        llm_model=_env_str("GRAPH_LLM_MODEL", "gpt-4o-mini"),
        llm_temperature=_env_float("GRAPH_LLM_TEMPERATURE", 0.0),
        schema_mode=_env_str("GRAPH_SCHEMA_MODE", "EXTRACTED").strip().upper(),
        embedding_model=_env_str("GRAPH_EMBEDDING_MODEL", "text-embedding-3-small"),
        build_batch_size=max(1, _env_int("GRAPH_BUILD_BATCH_SIZE", 1)),
        source_link_mode=_env_str("GRAPH_SOURCE_LINK_MODE", "from_chunk"),
        clean_before_build=_env_bool("GRAPH_CLEAN_BEFORE_BUILD", False),
    )
