...
def load_manifest(collections_dir: Path, collection_id: str) -> Dict[str, Any]:
    ...
    if not p.exists():
        return {"collection_id": collection_id, "created_at": iso_now(), "retrieval_mode": "vector", "docs": []}
    return read_json(p)

def set_retrieval_mode(manifest: Dict[str, Any], retrieval_mode: str) -> None:
    mode = (retrieval_mode or "vector").strip().lower()
    if mode not in {"vector", "hybrid"}:
        raise ValueError(f"Invalid retrieval_mode: {retrieval_mode}")
    manifest["retrieval_mode"] = mode
